...
//   }
  ...
  // Define the SolarPanelConfig interface
export interface SolarPanelConfig {
    yearlyEnergyDcKwh: number;  // DC energy produced by the panel in a year (in kWh)
  }
  
  // The function that finds the appropriate solar panel configuration
  export function findSolarConfig(
    solarPanelConfigs: SolarPanelConfig[], // Array of SolarPanelConfig
    yearlyKwhEnergyConsumption: number,    // User's yearly energy consumption in kWh
    panelCapacityRatio: number,            // The panel capacity ratio (efficiency, size factor, etc.)
    dcToAcDerate: number                   // Derating factor for DC to AC conversion
  ): number {
    // Find and return the index of the matching solar panel configuration
